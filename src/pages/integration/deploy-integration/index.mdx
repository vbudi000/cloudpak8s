...
14. **NOTE**: Leave the Persistent Volume Claim field blank. The HELM release will generate one for you. If you use an existing claim in use already, your container(s) will not start up in the pod. In the storage class field, enter a file based storage such as `ibmc-file-gold` or `ibmc-file-bronze.` Note that App Connect requires File (not block) based storage to work properly. 
