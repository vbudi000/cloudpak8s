...
Cloud Account with the ability to create Red Hat Open Shift clusters.  
It also requires a license to deploy the Cloud 
Pak for Integration.


## Deploying a managed OpenShift Cluster on IBM Cloud

6. Log into your IBM Cloud Account.  From the Dashboard on the Search Bar, 
enter in `Cloud Pak for Integration`.  It will take you to the section 
where you can create your own cluster.

7. Follow these steps to deploy Red Hat OpenShift and the Common 
Services -- 
[here](https://cloud.ibm.com/docs/cloud-pak-integration?topic=cloud-pak-integration-getting-started)
8. After you complete the installation process, it will create some 
namespaces as well as apply some general security settings required to install 
the Cloud Pak.  It will also automatically create a `Pull Secret` that you 
can use to deploy capabilities.  This secret is called 
`ibm-entitlement-key` and will be referenced multiple times.

## Helpful Hints

This section is designed to help with issues encountered with installing 
the Cloud Pak for Integration on ROKS (IBM Cloud). 

### Issues with Accessing Platform Navigator UI after install

In order to avoid issues with accessing the cp4i Platform Navigator, be sure 
...
wrong in the following URL. Instead the 0000 needs replaced with 0001:
...
`Schematics->Workspaces` and delete the old workspace. Do not delete the 
