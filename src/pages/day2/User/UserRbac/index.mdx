...
Note that we have disabled the **kubeadmin** in your environment as described in [here](../User/UserVmware). Therefore, the steps in this page must be performed using the system:admin user account which means that we will add **--user=admin** option with the oc command if necessary.
