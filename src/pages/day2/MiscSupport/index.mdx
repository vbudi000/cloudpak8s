...
description: OCP Day2 Miscellaneous - Support
keywords: 'ocp, day2, misc, support'
---
...
[Red Hat Suppot Portal](https://access.redhat.com/articles/38363)
# Mapping to Personas
...
[Here](https://github.ibm.com/CASE/OCP-Day2-operations/blob/master/Support/must-gather.txt) is the sameple output of the **oc adm must-gather** command.  
Note that the oc adm must-gather command will take a few munites to complete.    


There are a few ways to get the unique cluster ID.  
You can get it via the **oc get clusterversion** commmand as follow.
```
...
![support_4](/assets/img/day2/support_4.png)  
...
To solve the issue quicker, you need to understand how to open a support ticket and how to gather necessary data for problem determinattion.  We would recommend you to do a rehearsal if possible. 
...
Here is another example. If you kicked off the **oc adm must-gather** command from remote terminal and the network transfer is not high enough, it may take longer than you would expect.  We saw that it took an hour to complete the command. You probaly need to try the command to find out how long it will take because it will impact your SLA. You would gather data as quick as possible to send it to the support when you have an outage. Any incidents that cause outages or hits against the agreed upon SLAs should be capture and included within the platform performance measurements / your root cause analysis.  
