...
description: OCP Day2 Capacity
keywords: 'ocp, day2, capacity'
---
...
Some basic pattens (or best practises) for Capacity Management include:
...
Infrastructure capacity for OCP covers the traditional resources of an IaaS environment - Compute, Memory, Storage and other assets such as available network addresses so on.
...
During Day 2 operations, we want to allow regular changes to occur in the OCP cluster without risking a capacity failure.
...
The following series of articles **How Full is my Cluster** describe a Capacity Management in OCP 3.x, which is relevant for 4.x too:
...
In addition to the capabilities in OCP, OpenShift on IBM Cloud which is IBM's managed offering has the additional monitoring service with SysDig which can present potential capacity problems in infrastructure and applications.



## With IBM Cloudpak for MCM

In addition to the capabilities in OCP, IBM Cloud Pak for Multicloud Management has the additional monitoring tool IBM Cloud App Management (ICAM) which can present potential capacity problems in infrastructure and applications. ICAM specializes in monitoring and alerting against the golden signals, of which Saturation reflects capacity problems.
