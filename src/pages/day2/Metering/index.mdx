...
description: OCP Day2 Metering
keywords: 'ocp, day2, metering'
---
...
# Day 1 Platform
Metering can be a Day 1 or Day 2 activity depending on whether the business requirement for metering comes at the beginning of the OCP install or later.  If metering is considered as important by the business, it will be included in the design phase; then, it is a Day 1 activity.  However, some enterprises may only consider metering after they go to production.  In this case, metering is a Day 2 activity. 

OCP provides a `metering operator`; however, it is not installed by default. 
You can __install the metering operator__ via the OCP web console.
...
# Day 2 Platform
Depending on the defined data source, metering can report either usage of the OCP platform, the application, or both.
For platform metering, you can use the data source from the OCP configured Prometheus. So before you start using the metering operator, you need to __verify Prometheus is collecting data__. You can then __enable the metering operator's report__ on the cluster, node, pod, PVC resources for the request, usage, and other collected information for a different period.
...
As the application's data does not come preconfigured with the OCP configured Prometheus, you need to configure Prometheus to collect the required data.
...
Setting up metering is covered in the Application tasks section later in the chapter. Metering requires data, and for OCP, the data collection to generate report comes predominantly from the Prometheus that comes with OCP. You can only get a good metering report if the data are valid and comes regularly.  Therefore an important activity for Day 2 platform is to ensure that Prometheus is collecting data.
...
OCP Metering make use of the following 4 CRD (Custom Resource Definition)
...
Once you ascertain that you had the required compute capacity, you can perform the installation.  The installation steps are detailed in the [OCP Documentation](https://docs.openshift.com/container-platform/4.3/metering/metering-installing-metering.html), here is the summary:
...
At the end of the day, the reason that you perform metering is to produce a report per the business requirement. OCP provides the `metering operator`.  The `metering operator` comes with the `reporting operator`.  The `reporting operator` comes with some metering queries and reports.  However, the business requirement might require reports in a certain format that is different from what is provided out of the box.  A lot of the Day 2 activities on metering will be to generate queries and reports to fulfill the business requirement.
...
OCP comes with preconfigured _ReportDataSources_ and _ReportQueries_ that the user can start using.  _Report_ CRD is used to define the schedule of the report run.
...
The following is a link related to OCP metering.
...
- [IBM Cloudpak for MCM metering](https://www.ibm.com/support/knowledgecenter/SSFC4F_1.2.0/admin/metering_service_mcm.html)  
