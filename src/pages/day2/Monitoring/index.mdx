...
description: OCP Day2 Monitoring
keywords: 'ocp, day2, monitoring'
---
...
Out of the box, the Red Hat OpenShift Container Platform (OCP) provides the ability to monitor the OpenShift platform. As described below and at the links included herein, this capability should be leveraged and integrated with your enterprise event management and application performance management.

This section is based on OCP version 4.3. Please be sure to check the Red Hat documentation for the installed version at your location.
...
- Installation of the Monitoring Stack is default
  ...
  Implementation of the OCP Infrastructure Monitoring Stack is completed during OCP installation. Should there be a need for additional infrastructure related monitoring. Whether it be a third party solution from IBM or others, these tools should be planned and allocated for during this phase.
...
- Perform Customization of Monitoring Stack
  ...
  Note that customization of the default OCP Infrastructure Monitoring Stack is limited. Supported customizations include adding persistent storage to monitoring components, routing notification of the defined monitoring alerts and so on. Adding new alerts or changing the predefined dashboards is not supported (as of OCP 4.3).
...
| SRE | Redering data with the default Grafana dashboards |
...
Once deployed, the OCP Web Console displays a high level view of the current performance on the main OCP Dashboard. KPIs such as a Cluster Health Indicator, Capacity Utilization and availability across the cluster. High level cluster information,  alerts and top Pods, are displayed as well. This is a view only interface, meaning there are no integrations or extensions available.
![ocp_login_dashboard](/assets/img/day2/OCP_Login_Dashboard.png)

### Cluster Level Monitoring
When OCP is deployed, the Open Source monitoring and alerting tools, [**Prometheus**](https://prometheus.io/) the Prometheus [**Alert Manager**](https://prometheus.io/docs/alerting/overview/), and visualization software [**Grafana**](https://grafana.com/), are installed.
![prom_arch](/assets/img/day2/OCP_Prom_Arch.png)

Combined this software is referred to as the Monitoring Stack in the Open Shift documentation. This Stack provides immediate monitoring and visualization of the OCP deployment infrastructure components. Several Grafana Dashboards are supplied which provide various views of the time-series data collected by Prometheus. The Dashboard shown below leverages the [USE Method](http://www.brendangregg.com/usemethod.html) (Utilization, Saturation and Errors) for visualization of overall cluster performance:

![ocp_use](/assets/img/day2/Cluster_Use_Method.png)  

Some of the other dashboards included are:
- etcd - Metrics and etcd related performace.
...
![ocp_login_aaccess](/assets/img/day2/OCP_Auth_Window.png)



<a name="Using-Prometheus"></a>
### Viewing Prometheus metrics: [SRE]
OCP provides a Prometheus (like) UI for viewing and generating queries against the collected data. The UI is available under the Monitoring tab by selecting metrics:

![prom_ui](/assets/img/day2/Prom_OCP_UI.png)
...
![prom_ui](/assets/img/day2/OCP_PROM_QUERY.png)
...
Verify Built-in Grafana Dashboards are properly working by selecting dashboards under the monitoring tab in the OCP Dashboard hamburger menu.
Open one of the available dashboards and observe the performance of the OCP installation.  In this example we launched the etcd dashboard.

![ocp_grafana](/assets/img/day2/OCP_Grafana_Launch.png)

The OCP Grafana deployed with the Monitoring Stack does not allow for customizations. Should you desire to build your own dashboards (using this Prometheus data source) or need to import some of the [Open Shift Dashboards from the Grafana website](https://grafana.com/grafana/dashboards?search=OpenShift), please see [Red Hat Blog: Custom Grafana dashboards for Red Hat OpenShift Container Platform 4](https://www.redhat.com/en/blog/custom-grafana-dashboards-red-hat-openshift-container-platform-4).
...
![alrt_mgr](/assets/img/day2/OCP_PROM_AlrtMGR.png)
...
## OpenShift
The included monitoring stack in OCP is installed by default. The operations with OCP monitoring stack we described in this document come with OCP.


## OpenShift managed service on IBM Cloud  
You can find what solutions are available for monitoring and alerting on IBM Cloud in [here](./ibmcloud/README.md).  
...
More information on this CloudPak is [here](https://www.ibm.com/support/knowledgecenter/SSFC4F_1.2.0/kc_welcome_cloud_pak.html)

## Others Considerations

### Remote Health Monitoring [SRE]
OCP has a built in health monitoring system which reports data to Red Hat. This facility uses two integrated components: Telemetry and Insights Operator.
